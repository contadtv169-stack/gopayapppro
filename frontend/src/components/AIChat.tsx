...
Seja amigável, direto e útil. Responda em português do Brasil.
...
export function AIChat({ context = 'general', onSuggestion }: { context?: string; onSuggestion?: (text: string) => void }) {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { role: 'assistant', content: 'Olá! Sou o assistente IA do GoPay. Como posso ajudar? 💚' },
  ]);
  ...
  const sendMessage = async (content: string) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'assistant', content: '❌ Erro de conexão. Tente novamente ou contate suporte@gopay.com.br' }]);
    } finally {
      setLoading(false);
    }
  };
    ...
    <>
      {!open && (
        <button onClick={() => setOpen(true)}
          className="fixed bottom-6 right-6 z-50 w-14 h-14 bg-gradient-to-br from-go-500 to-primary-600 rounded-full shadow-xl 
          flex items-center justify-center hover:scale-105 active:scale-95 transition-all hover:shadow-go-500/30">
        ...
        <div className="fixed bottom-6 right-6 z-50 w-[380px] max-w-[calc(100vw-2rem)] bg-white rounded-2xl shadow-2xl border border-gray-100 flex flex-col overflow-hidden animate-in slide-in-from-bottom-4">
          ...
          <div className="bg-gradient-to-r from-go-500 to-primary-600 p-4 text-white flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-semibold text-sm">IA GoPay</p>
              ...
              <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[85%] rounded-2xl px-4 py-2.5 ${
                  ...
                  <div className="flex items-center gap-2 mb-1">
                    {msg.role === 'assistant' ? <Bot className="w-4 h-4 text-go-500" /> : <User className="w-4 h-4" />}
                    <span className="text-xs opacity-70">{msg.role === 'assistant' ? 'GoPay IA' : 'Você'}</span>
                  </div>
          ...
          <div className="p-3 border-t border-gray-100">
            ...
            <p className="text-[10px] text-gray-400 mt-1.5 text-center">Respostas geradas por IA • Verifique informações importantes</p>
          </div>
        </div>
      )}
    </>
