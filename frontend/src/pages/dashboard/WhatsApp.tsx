...
import { MessageCircle, Phone, Settings, Power, ExternalLink, Save, RefreshCw } from 'lucide-react';
import toast from 'react-hot-toast';
import { supabase } from '../../services/supabase';

export default function WhatsApp() {
  const [config, setConfig] = useState({ instanceId: '', token: '', enabled: false, autoReply: true, welcomeMessage: 'Ola! Bem-vindo(a) a {loja}! Como posso ajudar?' });
  ...
  useEffect(() => {
    (async () => {
      ...
      if (data) setConfig({ instanceId: data.instance_id || '', token: data.api_token || '', enabled: data.enabled || false, autoReply: data.auto_reply !== false, welcomeMessage: data.welcome_message || '' });
      setLoading(false);
    })();
  }, []);
  ...
  const saveConfig = async () => {
    ...
    await supabase.from('whatsapp_config').upsert({ user_id: uid, instance_id: config.instanceId, api_token: config.token, enabled: config.enabled, auto_reply: config.autoReply, welcome_message: config.welcomeMessage });
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        ...
        <span className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-sm font-medium ${config.enabled ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
          <Power className="w-4 h-4" />{config.enabled ? 'Ativo' : 'Inativo'}
        </span>
      </div>
      ...
      <div className="card !p-6 space-y-4">
        <div className="flex items-center gap-3 pb-4 border-b border-gray-100">
          <div className="w-10 h-10 bg-purple-100 rounded-xl flex items-center justify-center"><Settings className="w-5 h-5 text-purple-600" /></div>
          <div><h2 className="font-semibold text-gray-900">Respostas Automáticas</h2><p className="text-sm text-gray-500">Julia (IA) responde clientes automaticamente</p></div>
        </div>
        <label className="flex items-center gap-3">
          ...
          <span className="text-sm text-gray-700">Atender automaticamente com a Julia (IA)</span>
        </label>
        <div>
      ...
      <div className="card !p-6">
        ...
        <ol className="space-y-3 mt-4 text-sm text-gray-600">
          ...
          <li className="flex items-start gap-3"><span className="w-6 h-6 bg-go-100 text-go-700 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 mt-0.5">4</span>Ative respostas automáticas com Julia!</li>
        </ol>
      </div>
    </div>
  );
