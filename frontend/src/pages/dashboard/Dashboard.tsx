...
import { getDashboardStats, getOrders } from '../../services/supabaseData';
...
export default function Dashboard() {
  ...
  useEffect(() => {
    Promise.all([getDashboardStats(), getOrders()]).then(([s, r]) => {
      setStats(s);
          ...
          <div className="space-y-3">
            ...
            {recent.map((order: any) => (
              <div key={order.id} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
                <div>
                  <p className="text-sm font-medium text-gray-900">{order.customer_name || 'Pedido'}</p>
