...
async function getSellerProfile(sellerId: string) {
  ...
  try {
    const { data } = await supabase.from('users').select('business_name').eq('id', sellerId).maybeSingle();
    if (data?.business_name) return data.business_name;
  } catch {}
  return 'Vendedor';
}
...
async function processWithGateway(gateway: string, creds: any, params: {
  ...
  const baseUrl = GATEWAY_ENDPOINTS[gateway];
  ...
  if (!res.ok) throw new Error(`Gateway ${gateway} retornou erro ${res.status}`);
...
export async function getConnectedGateways(sellerId: string) {
  const { data } = await supabase.from('gateway_credentials').select('*').eq('user_id', sellerId).eq('is_active', true);
  return data || [];
}
...
export async function createCheckoutOrder(params: {
  ...
  try {
    if (gateway !== 'gopay') {
      ...
      if (gwCreds) {
        ...
        if (result) { pixCode = result.pixCode; qrUrl = result.qrUrl; }
      } else {
  ...
  if (!pixCode) {
    const pixBrCode = buildPixBRCode(params.amount, orderId, 'gopay', 'GoPay', 'BRASILIA');
    pixCode = pixBrCode;
    qrUrl = `https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=${encodeURIComponent(pixBrCode)}`;
  }

  const order = {
    id: orderId, user_id: params.seller_id, product_id: params.product_id || null,
    payment_link_id: params.payment_link_id || null, customer_name: params.customer_name,
    customer_email: params.customer_email || null, customer_phone: params.customer_phone || null,
    customer_document: params.customer_document || null, amount: params.amount,
    net_amount: params.amount - 7, status: 'pending', payment_method: 'pix', gateway,
    pix_code: pixCode, pix_qr: qrUrl, gateway_error: gatewayError || null,
    expires_at: new Date(Date.now() + 1200 * 1000).toISOString(), created_at: new Date().toISOString(),
  };
  ...
  try {
    ...
    if (error) {
      if (error.message?.includes('column') || error.code === '42703') {
        const minimalOrder = { id: orderId, user_id: params.seller_id, customer_name: params.customer_name,
  ...
  createOrderNotification(params.seller_id, {
    id: orderId, customer_name: params.customer_name, amount: params.amount, product_id: params.product_id,
  });
